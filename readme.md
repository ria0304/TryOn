...
POST /api/uploads/garment
        ↓
rembg background removal  +  category suggestion
        ↓
...
| Feature | Status |
|---|---|
| Garment upload + automatic background removal (`rembg`) | ✅ verified — 69-test backend suite runs the real `rembg` model (not mocked) end-to-end through the upload endpoint |
...
│   ├── requirements.txt
│   ├── pytest.ini
│   ├── tests/                        # 69 tests, 86% coverage — see Testing section below
│   ├── database/
│   │   ├── database.py
...
| `POST` | `/api/uploads/garment` | Upload a photo, get back original URL, `rembg` cutout URL, intelligent `warped` URL, and suggested category |
...
The backend has a pytest suite: **69 tests, 86% statement coverage**, run against real dependencies (not mocked) wherever feasible — the actual `rembg` model, actual MediaPipe BlazePose, actual scikit-learn KMeans clustering, all exercised through a real FastAPI `TestClient` hitting an isolated per-test SQLite database.
...
| `test_mannequin_manager.py`, `test_warping.py`, `test_layering.py` | TPS/affine warp math, alpha compositing, reference landmark geometry |
| `test_api_users_meta.py`, `test_api_garments.py`, `test_api_outfits.py`, `test_api_uploads.py` | Full FastAPI endpoints: CRUD, auth (401 vs 404 on stale ids), cross-user isolation, outfit exclusivity rules, and the **upload endpoint's full real pipeline** (rembg → classification → color detection, no mocks) |

**Known coverage gaps:**
- `classification.py`'s CLIP inference path itself (43% covered in this repo's suite) isn't exercised by these tests — only the decision-tree logic around it is — but the inference has been validated against real garment photos in the WYA project this logic was ported from
- `landmark_detector.py`'s MediaPipe internals (37% covered) — the wrapper is exercised indirectly through the upload pipeline test, but not unit-tested in isolation
