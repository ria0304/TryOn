...
from fabric_classification import classify_garment_fabric
from database import models
from database.schemas import UploadResult
from services.fitting_service import FittingService
from services.garment_segmentation import compute_flare, compute_top_ratio, extract_canonical_garment, extract_garment
...
async def upload_garment(
    ...
    if len(raw) > MAX_UPLOAD_BYTES:
        ...
        raise HTTPException(status_code=400, detail=f"File exceeds {limit_mb:.0f}MB limit")
    ...
    try:
        ...
        if not skip_background_removal:
            ...
            garment_only, _strict_alpha = extract_garment(person_cutout_np, category="dress")
        ...
        wya_category = detail.get("wya_category")
        ...
        resolved_category = resolve_category_with_geometry(
            suggested_category,
        ...
        if resolved_category != suggested_category:
            # Geometry overrode the vision label, so WYA's finer category
            # (still describing the pre-override label) is stale — fabric
            # classification should fall back to a generic category guess.
            wya_category = None
        ...
        if not skip_background_removal:
            ...
            garment_only, strict_alpha, asset_metadata = extract_canonical_garment(
                person_cutout_np, category=suggested_category
            )
            garment_only_image = Image.fromarray(garment_only, mode="RGBA")
            ...
            alpha_filename = f"mask-{stem}.png"
            alpha_path = CUTOUTS_DIR / alpha_filename
            Image.fromarray(strict_alpha, mode="L").save(alpha_path, format="PNG")
            canonical_asset = asset_metadata.to_dict()
            canonical_asset["url"] = cutout_url
            canonical_asset["alpha_mask_url"] = f"/static/cutouts/{alpha_filename}"

            # Use the final (completed) garment-only cutout for fitting --
            # the filled-in / mirrored silhouette is what we want to warp.
            cutout_buf = io.BytesIO()
            garment_only_image.save(cutout_buf, format="PNG")
            classification_source = cutout_buf.getvalue()

            # But for colour detection, use the STRICT mask instead: hole-fill
            # and left/right symmetry completion mark pixels opaque without
            # updating their RGB, so completed regions can carry real
            # background colour rather than garment colour. Sampling only
            # the pixels that were genuinely part of the original cutout
            # avoids that leak. See extract_garment()'s docstring.
            color_source_arr = garment_only.copy()
            color_source_arr[:, :, 3] = strict_alpha
            color_source_image = Image.fromarray(color_source_arr, mode="RGBA")
            color_source_buf = io.BytesIO()
            color_source_image.save(color_source_buf, format="PNG")
            color_detection_source = color_source_buf.getvalue()

        # 3. Dominant Color Detection — prefer the strict (uncompleted) mask
        # when we have one; classification_source has already been swapped
        # to a raw/unmasked image in the skip_background_removal path, so
        # fall back to it there since no strict mask exists.
        color_detection_input = color_detection_source if not skip_background_removal else classification_source
        suggested_color_hex, suggested_color_name = detect_dominant_color(color_detection_input)

        # 3a. Fabric Classification (texture + pattern + WYA's rule-based classifier)
        try:
            fabric_detail = classify_garment_fabric(
                classification_source,
                color_name=suggested_color_name,
                category=suggested_category,
                wya_category=wya_category,
            )
            suggested_fabric = fabric_detail["fabric"]
        except Exception as e:
            print(f"Fabric classification failed: {e}")
