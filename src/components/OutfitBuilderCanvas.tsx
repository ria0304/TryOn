...
import { Settings, X, Undo, Redo, Save, Plus, Camera } from 'lucide-react';
...
export const OutfitBuilderCanvas: React.FC<OutfitBuilderCanvasProps> = ({
  ...
  const [mannequinCanvas, setMannequinCanvas] = useState<HTMLCanvasElement | null>(null);
  ...
  const handleCapture3D = () => {
    if (!mannequinCanvas) return;
    const link = document.createElement('a');
    link.download = `tryon-3d-view-${Date.now()}.png`;
    link.href = mannequinCanvas.toDataURL('image/png');
    link.click();
  };
  ...
  return (
    <div className="bg-[#FFFFFF] border border-[#F3D3DB] rounded-3xl p-6 shadow-sm space-y-6 relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        {/* Mannequin Preview */}
        <div className="lg:col-span-5 relative bg-gradient-to-b from-[#FFF5F8] via-[#FFEBF1] to-[#FFDFE9] border border-[#F3D3DB] rounded-3xl p-4 flex items-center justify-center min-h-[560px] shadow-inner group overflow-hidden">
          {/* Top Control Bar with proper spacing */}
          <div className="absolute top-4 left-4 right-4 z-20 flex items-center justify-between pointer-events-none">
            <div className="px-3 py-1.5 rounded-full bg-white/90 backdrop-blur-md text-[10px] font-bold tracking-wider text-[#E97A9A] border border-[#F3D3DB] shadow-xs flex items-center gap-1.5 pointer-events-auto">
            ...
            <button
              ...
              disabled={!mannequinCanvas}
              className={`px-3.5 py-1.5 rounded-full text-[10px] font-bold tracking-wide border transition-all pointer-events-auto flex items-center gap-1.5 ${
                mannequinCanvas
                  ? 'bg-white/90 text-[#2F2A2E] border-[#F3D3DB] shadow-xs hover:bg-[#FFF0F4] hover:text-[#E97A9A] cursor-pointer'
                  : 'bg-white/50 text-gray-400 border-white/40 cursor-not-allowed'
              }`}
              title="Capture the current 3D view as a PNG"
            >
              <Camera className="w-3 h-3 text-[#E97A9A]" />
              <span>Capture 3D</span>
            </button>
          </div>

          <ThreeMannequin state={state} onCanvasReady={setMannequinCanvas} />
          
          {/* Category Quick Select (Overlay) */}
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 bg-white/90 backdrop-blur-md p-2 rounded-2xl border border-[#F3D3DB] shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            {categories.map((cat) => (
              <button
                key={cat.id}
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="bg-[#FFFFFF] border border-[#F3D3DB] rounded-2xl p-5 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-[10px] font-bold text-[#6D6670] uppercase tracking-wider">Current Selection</h3>
              <span className="text-[10px] font-bold text-[#E97A9A] bg-[#FFF0F4] px-2 py-0.5 rounded-full border border-[#F3D3DB]">
                3D Back Auto-Mapped
              </span>
            </div>
            ...
            <div className="space-y-3">
              {ALL_CATEGORIES.map((cat) => {
                ...
                if (!item) return null;
                const hasStrapInfo = item.strapType || item.analysis?.strapType;
                return (
                  <div key={cat} className="p-2.5 bg-[#FFF8FA] rounded-xl border border-[#F3D3DB]/60 space-y-1.5">
                    <div className="flex items-center justify-between">
                    ...
                    {(cat === 'top' || cat === 'dress' || cat === 'jacket') && (
                      <div className="flex items-center justify-between pt-1 border-t border-[#F3D3DB]/40 text-[9px]">
                        <span className="text-[#6D6670]">Strap / Back Structure:</span>
                        <span className="font-semibold text-[#BE185D]">
                          {item.strapType === 'thin_double_straps'
                            ? 'Delicate Double Straps (Open Back)'
                            : item.strapType === 'halter_neck'
                            ? 'Halter Neck (Open Back Tie)'
                            : item.strapType === 'strapless'
                            ? 'Strapless / Backless'
                            : item.strapType === 'crossed_straps'
                            ? 'Criss-Cross Back'
                            : item.backStyle === 'crossed_back'
                            ? 'Criss-Cross Back'
                            : item.backStyle === 'open_back'
                            ? 'Open Back'
                            : 'Wide Straps (Covered Back)'}
                        </span>
                      </div>
                    )}
                  </div>
